...
public interface PlatformGatewayFeignClient {
  ...
  /**
   * Perform GET operation for searching data factory entities by query params in different
   * registry
   *
   * @param registryTarget another registry to search
   * @param resource       url resource
   ...
   * @see ConnectorResponse
   */
  @GetMapping(path = "/data-factory/{registryTarget}/{resource}",
      produces = MediaType.APPLICATION_JSON_VALUE)
  ConnectorResponse performSearch(@PathVariable("registryTarget") String registryTarget,
      @PathVariable("resource") String resource, @SpringQueryMap Map<String, String> params,
      @RequestHeader HttpHeaders headers);
